...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            nl = len(labels)
            tcls = labels[:, 0:2].tolist() if nl else [[]]  # target class
            seen += 1
            ...
            if nl:
                ...
                # tcls_tensor = labels[:, 0:2]
                # Per target class
                # for cls in torch.unique(tcls_tensor, dim=0):
                    # cls = cls.unsqueeze(0)
                    # ti = (cls == tcls_tensor).all(dim=-1).nonzero().view(-1)  # target indices
                    # pi = (cls == pred[:, 5]).all(dim=-1).nonzero().view(-1)  # prediction indices

                for i in range(2):
                    tcls_tensor = labels[:, i]
